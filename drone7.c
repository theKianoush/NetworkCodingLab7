...
// PROBLEMS:
// - Unfortunately my program does not hanlde duplicates or duplicate ACKs
// - i only did the first half of the assignment which was implementing the move function 
// - first i send to location 2 and it changes to location7, the locations are also swapped when it ACKs back
// - and then we send a second message to see if it will arrive at the final location 15, and it does just that, but at the last port, it displays myLocation improperly, but you can see it is at the right port 
